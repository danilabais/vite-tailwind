<template>
  <section class="ex select-none">
    ...
    <div class="ex__input-wrap flex items-center flex-col md:flex-row mb-8">
      ...
      <inputCurrency :propValue="est" :propForState="minAm" :choosedCurr="choosedCurr2" @changeCurr="changeCurr('choosedCurr2',$event)" class="ex__select"/>

    </div>
    <h4 class="mb-2">
      Your Etheruem Address
    </h4>
    <div class="flex md:flex-row flex-col  w-100">
        <input class="rounded-md flex-1 mr-0   md:mr-8 bg-slate-100 border  border-gray-300 placeholder:text-gray-400 focus:border-gray-300  focus:ring-0 placeholder:pr-10"   type="text">
        <button class="py-2 relative md:mt-0 mt-4 uppercase hover:bg-sky-600 transition-all  rounded-md px-14 bg-sky-400 text-white">
          <span @click="submit" class="font-medium">exchange</span>
          <span :class="{'opacity-0': !error}" class="text-red-600 top-full pt-2 normal-case pointer-events-none text-sm text-center  left-0 right-0 absolute">This pair is disabled now</span>
        </button>
    </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.est===null || this.minAm===null) {
        this.error = true
